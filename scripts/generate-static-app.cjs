...
const input = process.argv[2] || path.join(process.cwd(), 'data', 'configurations.json');
...
fs.writeFileSync(path.join(outDir, 'package.json'), JSON.stringify({
  name: (config.businessName || 'generated-app').toLowerCase().replace(/\s+/g,'-'),
  private: true,
  scripts: {
    dev: 'vite',
    build: 'vite build',
...
// public/index.html
fs.writeFileSync(path.join(outDir, 'public', 'index.html'), `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
...
// src/main.jsx
fs.writeFileSync(path.join(outDir, 'src', 'main.jsx'), `import React from 'react'
...
fs.writeFileSync(path.join(outDir, 'src', 'App.jsx'), `import React from 'react'
...
// styles.css
fs.writeFileSync(path.join(outDir, 'src', 'styles.css'), `body { margin: 0; font-family: Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial; }
`);

// config.json copy
fs.writeFileSync(path.join(outDir, 'src', 'config.json'), JSON.stringify(config, null, 2));
