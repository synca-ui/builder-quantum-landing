  ...
  try {
    ...
    try {
      ...
      if ((prisma as any).floorPlan) {
        ...
        console.log(
          "⚠️  Dashboard tables not available in current Prisma client",
        );
        console.log(
          "   The API endpoints will use mock data until tables are created",
        );
    ...
    console.log(
      `   - GET /api/dashboard/insights/overview?businessId=${businessId}`,
    );
    console.log(
      `   - GET /api/dashboard/floor-plan/plans?businessId=${businessId}`,
    );
  } catch (error) {
...
async function seedDashboardTables(businessId: string) {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    reservationTime.setDate(
      reservationTime.getDate() + Math.floor(Math.random() * 7),
    ); // Next 7 days
    ...
    await (prisma as any).reservation.create({
      data: {
        businessId: businessId,
        guestName:
          reservationNames[Math.floor(Math.random() * reservationNames.length)],
  ...
  const menuItems = [
    ...
    { name: "Caesar Salad", price: 9.8 },
    { name: "Grilled Salmon", price: 18.9 },
    { name: "Pasta Carbonara", price: 14.2 },
    { name: "Beef Burger", price: 13.5 },
    { name: "Tiramisu", price: 6.5 },
    { name: "Wine (Glass)", price: 7.0 },
  ];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const tip =
      Math.random() > 0.3 ? subtotal * (0.05 + Math.random() * 0.1) : 0; // 5-15% tip
    ...
    await (prisma as any).order.create({
      data: {
        businessId: businessId,
        tableId:
          createdTables[Math.floor(Math.random() * createdTables.length)].id,
