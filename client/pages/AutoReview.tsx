...
import {
  ArrowLeft, ChevronRight, CheckCircle2, Globe, Phone, Mail,
  Instagram, Utensils, Clock, Star, Image as ImageIcon,
  Sparkles, Edit3, Palette, Type, MapPin, Check, Info,
  Building, FileText,
} from "lucide-react";
...
// ─────────────────────────────────────────────────────────────────────────────
const FONT_OPTIONS = [
  { id: "sans-serif", name: "Sans Serif", class: "font-sans", preview: "Modern & Clean",   description: "Perfekt für digitale Lesbarkeit" },
  { id: "serif",      name: "Serif",      class: "font-serif", preview: "Klassisch & Elegant", description: "Traditionell und anspruchsvoll" },
  { id: "mono",       name: "Display",    class: "font-mono",  preview: "Bold & Kreativ",  description: "Auffällig und einzigartig" },
];

const TEMPLATE_OPTIONS = [
  { id: "minimalist", name: "Minimalist", description: "Klar, aufgeräumt, modern" },
  { id: "classic",    name: "Classic",    description: "Warm, einladend, traditionell" },
...
const BUSINESS_TYPES = [
  ...
  "sushi", "steakhouse", "bistro", "imbiss", "foodtruck",
];
          ...
          <React.Fragment key={i}>
            <div className="flex flex-col items-center gap-1.5">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-all ${
                ...
                active ? "bg-white border-teal-500 shadow-md" :
              ...
              <span className={`text-xs font-semibold ${active ? "text-teal-600" : done ? "text-gray-500" : "text-gray-300"}`}>
                {s.label}
              </span>
            </div>
            {i < STEPS_NAV.length - 1 && (
              <div className={`h-0.5 w-16 mx-1 mb-5 transition-all ${i < current ? "bg-teal-500" : "bg-gray-200"}`} />
            )}
          </React.Fragment>
        );
      })}
...
export default function AutoReview() {
  ...
  const set = useCallback(<K extends keyof ReviewForm>(k: K, v: ReviewForm[K]) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/scraper-job/full?websiteUrl=${encodeURIComponent(wsUrl)}`)
      ...
      .then(data => { if (data?.job) { setJob(data.job); prefill(data.job); } })
      .catch(() => {})
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast({ title: "Bereit! 🎉", description: "Deine Konfiguration wird geladen." });
      ...
      <div className="max-w-4xl mx-auto px-5 pt-4 pb-20">
        ...
        <div className="text-center mb-10">
          <div className="flex items-center justify-center gap-2 mb-3">
            ...
            {job?.maitrScore && (
              <div className="flex items-baseline gap-1 px-3 py-1 rounded-full bg-teal-50 border border-teal-100">
                <span className="text-lg font-black text-teal-600">{job.maitrScore}</span>
                <span className="text-xs text-teal-400 font-medium">/100 Maitr Score</span>
              </div>
            )}
          </div>
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Wir haben alles automatisch extrahiert. Überprüfe die Daten und passe sie nach Bedarf an.
          </p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                <Building className="w-5 h-5 text-teal-600" /> Basis-Informationen
              </h3>
              <div className="grid sm:grid-cols-2 gap-6">
                <FormField label="Restaurantname *" value={form.businessName} onChange={v => set("businessName", v)} placeholder="z.B. Kleiner Kiepenkerl" />
                <div>
                ...
                <FormField label="Slogan" value={form.slogan} onChange={v => set("slogan", v)} placeholder="Kurzer einprägsamer Satz…" />
                <FormField label="Standort" value={form.location} onChange={v => set("location", v)} placeholder="Münster, NRW" />
                <FormField label="Telefon" value={form.phone} onChange={v => set("phone", v)} placeholder="+49 251 …" />
                <FormField label="E-Mail" value={form.email} onChange={v => set("email", v)} placeholder="info@…" />
              </div>
              <div className="mt-6">
            ...
            {job?.analysisFeedback && (
              <div className="bg-purple-50 border border-purple-100 rounded-2xl p-6">
                ...
                <p className="text-sm text-purple-700 leading-relaxed">{job.analysisFeedback}</p>
              </div>
            )}
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-6">
                ...
                {TEMPLATE_OPTIONS.map(t => (
                  ...
                  >
                    <CardContent className="p-5 text-center">
                      <div className="font-bold text-sm text-gray-900 mb-1">{t.name}</div>
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
                    ...
                    {extractedColors.slice(0, 8).map((c: string, i: number) => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-xl border-2 transition-all hover:scale-110 ${
                          form.primaryColor === c ? "border-teal-500 shadow-md scale-110" : "border-white shadow"
                        }`}
                        style={{ background: c }}
                      />
                    ))}
              ...
              {/* Color Presets */}
              <div className="mb-6">
                <p className="text-sm font-bold text-gray-700 mb-3">Schnell-Presets</p>
              ...
              <div className="grid sm:grid-cols-2 gap-6 lg:gap-8">
                <ColorInput label="Primärfarbe (Buttons, CTAs)"     value={form.primaryColor}    onChange={v => set("primaryColor", v)}    />
                <ColorInput label="Sekundärfarbe (Akzente)"          value={form.secondaryColor}  onChange={v => set("secondaryColor", v)}  />
                <ColorInput label="Hintergrundfarbe"                 value={form.backgroundColor} onChange={v => set("backgroundColor", v)} />
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                ...
                Speisekarte
                {menuItems.length > 0 && (
                  <span className="ml-2 text-sm font-normal text-gray-400">· {menuItems.length} Gerichte extrahiert</span>
                )}
              </h3>
              {menuItems.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto pr-1">
                  {menuItems.slice(0, 30).map((item: any, i: number) => (
                    <div key={i} className="flex items-center justify-between bg-white rounded-xl px-4 py-2.5 border border-gray-100">
                      ...
                      {item.price && (
                        <span className="text-sm font-bold text-gray-500 shrink-0 ml-3">{item.price}</span>
                      )}
                    </div>
                  ))}
                  {menuItems.length > 30 && (
                    <p className="text-xs text-gray-400 px-4 py-2">+{menuItems.length - 30} weitere Gerichte</p>
                  )}
                </div>
              ) : (
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="text-xs mt-1">Du kannst sie später im Konfigurator manuell hinzufügen.</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                ...
                Fotos
                {photos.length > 0 && (
                  <span className="ml-2 text-sm font-normal text-gray-400">· {photos.length} gefunden</span>
                )}
              </h3>
              {photos.length > 0 ? (
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {photos.slice(0, 12).map((src: string, i: number) => (
                    <div key={i} className="aspect-square rounded-xl overflow-hidden bg-gray-200 border border-white shadow-sm">
                      <img src={src} alt="" className="w-full h-full object-cover" loading="lazy" />
                    </div>
                  ))}
                  {photos.length > 12 && (
                    <div className="aspect-square rounded-xl bg-gray-100 border border-gray-200 flex items-center justify-center">
                      <span className="text-xs font-bold text-gray-400">+{photos.length - 12}</span>
                    </div>
                  )}
                </div>
              ) : (
                <div className="text-center py-8 text-gray-400">
                  ...
                  <p className="text-xs mt-1">Du kannst sie später manuell hochladen.</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {[
                  ...
                  { icon: Instagram, label: "Instagram",  value: job?.instagramUrl },
                  { icon: Utensils,  label: "Speisekarte", value: job?.menuUrl     },
                  { icon: Clock,     label: "Reservierung", value: job?.hasReservation ? "System erkannt" : undefined },
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {[
                  ...
                  { label: "Menü-Gerichte", value: menuItems.length ? `${menuItems.length} extrahiert` : "Manuell hinzufügen" },
                  { label: "Fotos",         value: photos.length      ? `${photos.length} extrahiert`  : "Manuell hochladen"  },
              ...
              <div className="mt-6 pt-6 border-t border-gray-200">
                <p className="text-sm font-bold text-gray-700 mb-3">Farbpalette</p>
                <div className="flex gap-3">
                  {[form.primaryColor, form.secondaryColor, form.backgroundColor].map((c, i) => (
                    <div key={i} className="flex flex-col items-center gap-1">
                      <div className="w-10 h-10 rounded-xl border border-gray-200 shadow-sm" style={{ background: c }} />
            ...
            <div className="bg-gradient-to-br from-teal-50 to-purple-50 border border-teal-100 rounded-2xl p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">Alles bereit!</h3>
              <p className="text-sm text-gray-600 mb-6 max-w-sm mx-auto">
                Deine App wird mit allen extrahierten Daten erstellt. Du kannst danach noch alles weiter anpassen.
              </p>
              <Button
