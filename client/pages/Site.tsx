...
const FALLBACK_CONFIG: Configuration = {
  ...
  menuItems: [
    ...
    },
    {
      id: "croissant",
      name: "Croissant",
      description: "Buttery and flaky, baked fresh",
      price: 2.5,
      imageUrl: "/placeholder.svg",
    },
  ],
...
function SiteRenderer({ config: formData }: { config: Configuration }) {
  ...
  const activePage = (
    segs.length > 2 ? segs[segs.length - 1] : "home"
  ).toLowerCase();
  ...
  const themeOverride =
    (formData as any).templateThemes?.[selectedIdForSwitch] || {};
  ...
  const renderPageContent = () => {
    ...
    const gallery = (formData.gallery || []).map((g: any) => ({
      ...g,
      ...
      case "home":
        return (
          <section className="pb-24">
            <OffersBanner
              offers={(formData as any).offers}
              styles={styles}
              normalizeUrl={normalizeUrl}
              offerBanner={(formData as any).offerBanner}
            />
            <div className="px-6 pt-6">
              ...
              <h1 className="text-center text-xl font-extrabold mt-3">
                {getBusinessName()}
              </h1>
              {formData.slogan && (
              ...
              <div className="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {items.slice(0, 4).map((it: any, index: number) => (
                  <button
                    ...
                    className="text-left rounded-2xl border border-white/40 bg-white/25 backdrop-blur-md p-3 shadow-lg"
                    onClick={() => setProductOpen(it)}
                  >
                    {(formData as any).homepageDishImageVisibility !==
                      "hidden" && (
                      <img
                        src={normalizeUrl(it.image)}
                        alt={it.name}
                    ...
                    <div
                      ...
                      style={{ color: toRgba(String(styles.userPrimary), 0.9) }}
                    >
                  ...
                  <div className="mt-4 rounded-2xl border border-white/30 bg-white/15 backdrop-blur-md p-4 text-white/95">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => setOpenHoursExpanded((v) => !v)}
                      >
                    ...
                    <div className="mt-2 text-xs space-y-1">
                      {(openHoursExpanded
                        ...
                        : Object.entries(formData.openingHours).slice(0, 2)
                      ).map(([day, hours]: any) => (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {items.map((it: any, index: number) => {
                ...
                return (
                  <div
                    key={it.id || it.name || index}
                    className="rounded-2xl border border-white/40 bg-white/15 backdrop-blur-md p-3 shadow-lg cursor-pointer relative"
                    onClick={() => setProductOpen(it)}
                  >
                    {/* Social proof badge */}
                    ...
                    {typeof it.price !== "undefined" && (
                      <div
                        className="text-xs"
                        style={{
                          color: toRgba(String(styles.userPrimary), 0.9),
                        }}
                      >
                        ${Number(it.price).toFixed(2)}
                      </div>
        ...
        return (
          <section className="max-w-md mx-auto px-6 py-8">
            ...
            <GalleryGrid
              ...
              templateStyle={selectedIdForSwitch}
            />
          </section>
        );
      case "about":
  ...
  return (
    ...
    >
      ...
      <header className="px-4 h-14 flex items-center justify-between relative z-30 bg-white">
        <Link
          to={pageLink("home")}
          className="flex items-center gap-2 text-lg font-extrabold text-black"
        >
          <span className="w-8 h-8 rounded-xl inline-flex items-center justify-center overflow-hidden bg-gray-100">
            <LogoDisplay />
          </span>
          {getBusinessName()}
        </Link>
        <button
          ...
          className="sm:hidden w-9 h-9 rounded-xl flex items-center justify-center bg-gray-100 text-black"
        >
          ...
          {menuPages
            ...
            .map((p) => (
              <Link
                key={p}
                to={pageLink(p)}
                className={`${activePage === p ? "text-black" : "text-gray-600"} hover:text-black font-semibold`}
              >
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-20">
          <div
            ...
            style={{
              top: "calc(max(env(safe-area-inset-top, 50px), 50px) + 56px)",
            }}
            onClick={() => setMenuOpen(false)}
          />
          <div
            className="fixed left-0 right-0 bg-white border-b border-black/10 shadow-lg"
            style={{
              top: "calc(max(env(safe-area-inset-top, 50px), 50px) + 56px)",
            }}
          >
            <div className="px-4 py-3 flex justify-end">
              <button
                ...
                className="w-9 h-9 rounded-xl bg-gray-100 text-black"
                onClick={() => setMenuOpen(false)}
              >
                ×
              </button>
            ...
            <div className="px-4 pb-4 space-y-2">
              {menuPages.map((p) => (
                <Link
                  ...
                  to={pageLink(p)}
                  onClick={() => setMenuOpen(false)}
                  className={`block w-full text-left px-4 py-3 rounded-xl font-semibold ${activePage === p ? "bg-gray-100 text-black" : "text-gray-800 hover:bg-gray-50"}`}
                >
                  {p.charAt(0).toUpperCase() + p.slice(1)}
                </Link>
              ))}
      ...
      <div className="pb-20">{renderPageContent()}</div>
      ...
      {productOpen && (
        <div className="absolute inset-0 z-30 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white text-gray-900 rounded-2xl w-full max-w-sm p-5 shadow-2xl">
            ...
            <img
              src={normalizeUrl(productOpen.image)}
            ...
            {typeof productOpen.price !== "undefined" && (
              ...
              >
                ${Number(productOpen.price).toFixed(2)}
              </div>
            )}
            <button
              className="w-full rounded-lg text-white py-2 font-medium"
      ...
      {activePage === "home" && formData.reservationsEnabled && (
        <div className="fixed bottom-4 left-0 right-0 flex justify-center px-4 z-20">
          <ReservationButton
            color={
              (formData as any).reservationButtonColor || styles.userPrimary
            }
            textColor={
              (formData as any).reservationButtonTextColor || "#FFFFFF"
            }
