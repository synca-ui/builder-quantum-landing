            ...
            <div className="grid grid-cols-3 gap-4">
              {testCases.map(subdomain => (
                <Button
                  ...
                  disabled={loading}
                  variant={subdomain === 'nonexistent' ? 'destructive' : 'default'}
                >
                  Test: {subdomain}
                </Button>
              ))}
                ...
                <div key={index} className={`p-4 rounded border ${result.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                  ...
                  {result.success ? (
                    <div className="text-sm">
                      <p><strong>Business:</strong> {result.data?.businessName || 'N/A'}</p>
                      <p><strong>Template:</strong> {result.data?.template || 'N/A'}</p>
                      <p><strong>Status:</strong> {result.data?.status || 'N/A'}</p>
                      <p><strong>Published URL:</strong> {result.data?.publishedUrl || 'N/A'}</p>
                    </div>
                  ) : (
