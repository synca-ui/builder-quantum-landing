  ...
  const loadConfigurations = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        setError(
          handleApiError(result.error || "Failed to load configurations"),
        );
      }
    } catch (err) {
  ...
  const deleteConfiguration = async (id: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(
          handleApiError(result.error || "Failed to delete configuration"),
        );
      }
    } catch (err) {
  ...
  const publishConfiguration = async (id: string) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setError(
          handleApiError(result.error || "Failed to publish configuration"),
        );
      }
    } catch (err) {
        ...
        {configurations.length === 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-8 max-w-md mx-auto">
              Create your first website with our easy-to-use builder. It only
              takes a few minutes!
            </p>
            <Link to="/configurator">
              <Button
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  Manage and publish your {configurations.length} website
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="text-2xl">
                          {getBusinessTypeIcon(config.businessType)}
                        </div>
                        <div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(config.status || "draft")}`}
                        >
                          {config.status || "draft"}
                        </span>
                        {config.publishedUrl && (
                      ...
                      <div className="flex space-x-2 pt-4 border-t border-gray-100">
                        <Link
                          to={`/configurator?edit=${config.id}`}
                          className="flex-1"
                        >
                          <Button
                            variant="outline"
                        ...
                        config.publishedUrl ? (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              window.open(config.publishedUrl, "_blank")
                            }
                            className="flex-1"
                          >
