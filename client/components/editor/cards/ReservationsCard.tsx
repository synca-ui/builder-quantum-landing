...
export function ReservationsCard({ settings = {}, onChange }: ReservationsCardProps) {
  const [reservations, setReservations] = useState<ReservationSettings>(settings || {
    enabled: false,
    minPartySize: 1,
    maxPartySize: 50,
    timeSlotDuration: 60,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        />
        <label className="text-sm font-medium text-gray-700">Enable Table Reservations</label>
      </div>
      ...
      {reservations.enabled && (
        <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Min Party Size</label>
              <input
                type="number"
                value={reservations.minPartySize || 1}
                onChange={(e) => handleChange({ minPartySize: parseInt(e.target.value) || 1 })}
                min="1"
                max="100"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange({ maxPartySize: parseInt(e.target.value) || 50 })}
                min="1"
                max="500"
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => handleChange({ timeSlotDuration: parseInt(e.target.value) || 60 })}
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange({ advanceBookingDays: parseInt(e.target.value) || 30 })}
                min="1"
                max="365"
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
          ...
          <div>
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">Require Deposit for Reservations</span>
            </label>
          </div>
