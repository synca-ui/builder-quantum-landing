...
const FEATURE_LIST = [
  {
    ...
    description: 'Allow customers to place orders online with payment processing',
    icon: '🛒'
  },
  {
    id: 'onlineStore',
...
export function AdvancedFeaturesCard({ features = {}, onChange }: AdvancedFeaturesCardProps) {
  const [enabledFeatures, setEnabledFeatures] = useState<Partial<AdvancedFeatures>>(features || {});
    ...
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 flex gap-2">
        ...
        <p className="text-sm text-blue-800">
          Enable features to unlock advanced capabilities for your site. Some features may require a higher subscription tier.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {FEATURE_LIST.map((feature) => (
          ...
          >
            <input
              type="checkbox"
              checked={enabledFeatures[feature.id as keyof AdvancedFeatures] || false}
      ...
      {Object.values(enabledFeatures).some(v => v) && (
        <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-sm text-green-800 font-medium">
            ✓ {Object.values(enabledFeatures).filter(v => v).length} feature{Object.values(enabledFeatures).filter(v => v).length !== 1 ? 's' : ''} enabled
          </p>
        </div>
      )}
    </div>
