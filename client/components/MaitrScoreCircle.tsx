...
function getRating(s: number) {
  if (s >= 86) return { label: "Ausgezeichnet", accent: "#059669", pill: "rgba(5,150,105,0.1)", border: "rgba(5,150,105,0.22)" };
  if (s >= 71) return { label: "Gut", accent: "#0891b2", pill: "rgba(8,145,178,0.1)", border: "rgba(8,145,178,0.22)" };
  if (s >= 51) return { label: "Befriedigend", accent: "#d97706", pill: "rgba(217,119,6,0.1)", border: "rgba(217,119,6,0.22)" };
  return { label: "Verbesserungsbedarf", accent: "#ea580c", pill: "rgba(234,88,12,0.1)", border: "rgba(234,88,12,0.22)" };
}

interface BI { label: string; value: number; max: number; color: string; }
...
function getBreakdown(score: number): BI[] {
  ...
  raws
    ...
    .sort((a, b) => b.f - a.f)
    .forEach(({ i }) => { if (rem > 0 && floors[i] < cats[i].max) { floors[i]++; rem--; } });
...
function Bar({ label, value, max, color, delay }: BI & { delay: number }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <span style={{ minWidth: 120, fontSize: 11.5, color: "#475569", fontWeight: 500 }}>{label}</span>
  ...
  const inner = (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{ fontSize: 12.5, color: "#1e293b", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginTop: 1 }}>
          {value}
        </div>
      </div>
      {href && <ExternalLink style={{ width: 12, height: 12, color: "#cbd5e1", flexShrink: 0 }} />}
    </div>
  );
...
export default function MaitrScoreCircle({
  ...
  const analysisChips = [
    websiteUrl && { icon: <Globe size={15} />, label: "Website", value: websiteUrl, href: websiteUrl },
    extractionTime && { icon: <Clock size={15} />, label: "Analysiert am", value: extractionTime, href: undefined },
    processingTime && { icon: <TrendingUp size={15} />, label: "Dauer", value: processingTime, href: undefined },
  ].filter(Boolean) as { icon: React.ReactNode; label: string; value: string; href?: string }[];

  const dataChips = [
    businessName && { icon: <Globe size={15} />, label: "Unternehmen", value: businessName, href: undefined },
    businessType && { icon: <Utensils size={15} />, label: "Typ", value: businessType, href: undefined },
    phone && { icon: <Phone size={15} />, label: "Telefon", value: phone, href: undefined },
    email && { icon: <Mail size={15} />, label: "E-Mail", value: email, href: undefined },
    instagramUrl && { icon: <Instagram size={15} />, label: "Instagram", value: "Profil gefunden", href: instagramUrl },
    menuUrl && { icon: <Utensils size={15} />, label: "Speisekarte", value: "Online verfügbar", href: menuUrl },
    hasReservation && { icon: <Clock size={15} />, label: "Reservierung", value: "System erkannt", href: undefined },
  ].filter(Boolean) as { icon: React.ReactNode; label: string; value: string; href?: string }[];
  ...
  if (isLoading) {
    return (
      <div style={{
        ...
        boxShadow: "0 16px 48px rgba(0,0,0,0.10), 0 2px 0 rgba(255,255,255,0.95) inset",
        padding: "28px 24px",
        display: "flex", alignItems: "center", justifyContent: "center",
    ...
    <>
      ...
      <div className="ms-in ms-card-lift" style={{
        ...
        <div aria-hidden style={{
          ...
          <div style={{ position: "absolute", width: 320, height: 320, bottom: -80, right: 80, borderRadius: "50%", background: "radial-gradient(circle,rgba(221,214,254,0.7) 0%,transparent 68%)", animation: "blob2 13s ease-in-out infinite" }} />
          <div style={{ position: "absolute", width: 260, height: 260, top: 20, right: 220, borderRadius: "50%", background: "radial-gradient(circle,rgba(254,215,170,0.6) 0%,transparent 68%)", animation: "blob3 15s ease-in-out infinite" }} />
        ...
        <div style={{
          ...
          {/* Label */}
          <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 10 }}>
            Maitr Score
          </div>

          {/* Zahl + /100 + Rating-Badge — alles in einer Zeile */}
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: businessName ? 6 : 16 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
              <span style={{ fontSize: 62, fontWeight: 900, color: "#0f172a", lineHeight: 1, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>
                {displayScore}
              </span>
              <span style={{ fontSize: 17, fontWeight: 600, color: "#cbd5e1" }}>/100</span>
            </div>
            <span style={{
              fontSize: 12.5, fontWeight: 700,
              color: rating.accent, background: rating.pill,
          ...
          {businessName && (
            <p style={{ fontSize: 13, color: "#475569", fontWeight: 600, margin: "0 0 16px" }}>
              {businessName}
            </p>
          )}
      ...
      {(analysisChips.length > 0 || dataChips.length > 0) && (
        <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 22 }}>
          ...
          {analysisChips.length > 0 && (
            <div>
              <p style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" }}>
                Analyse Details
              </p>
          ...
          {dataChips.length > 0 && (
            <div>
              <p style={{ fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 10px" }}>
                Extrahierte Datenpunkte · <span style={{ color: "#06b6d4" }}>{dataChips.length} gefunden</span>
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                {dataChips.map((c, i) => (
