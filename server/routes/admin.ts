 ...
 */
router.put('/reservations/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    const userId = req.userId;
  ...
  try {
    ...
    const seoAnalysis = {
      ...
      checklist: {
        ...
        hasMenu: {
          status: config?.menuItems && Array.isArray(config.menuItems) && config.menuItems.length > 0,
    ...
    seoAnalysis.score = Math.round((completedItems / checklistItems.length) * 100);

    // Generate recommendations
    checklistItems.forEach(item => {
      if (!item.status) {
        seoAnalysis.recommendations.push(`Complete: ${item.title} - ${item.description}`);
      }
    });

    // Add specific SEO issues
    if (seoAnalysis.score < 50) {
      seoAnalysis.issues.push('Low SEO completeness may hurt search visibility');
    }
    if (!business.cuisine) {
      seoAnalysis.issues.push('Missing cuisine type affects local search results');
    }
    if (!config?.publishedUrl) {
      seoAnalysis.issues.push('Website not published - no search engine visibility');
    }
...
router.get('/analytics-summary', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [reservationCount, staffCount, tableCount, orderCount] = await Promise.all([
      (prisma as any).reservation?.count({ where: { businessId } }) || 0,
      (prisma as any).staff?.count({ where: { businessId, isActive: true } }) || 0,
...
router.get('/system-health', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const healthChecks = {
      ...
      qrCodes: { status: 'unknown', message: 'Checking QR code generation...' },
      reservations: { status: 'unknown', message: 'Checking reservation system...' },
    };
    ...
    try {
      ...
      if (latestAnalytics) {
        const daysSinceUpdate = Math.floor((Date.now() - latestAnalytics.date.getTime()) / (1000 * 60 * 60 * 24));
        if (daysSinceUpdate <= 1) {
          healthChecks.analytics = { status: 'healthy', message: 'Analytics data up to date' };
        } else {
          healthChecks.analytics = { status: 'warning', message: `Analytics data ${daysSinceUpdate} days old` };
        }
      } else {
        healthChecks.analytics = { status: 'error', message: 'No analytics data found' };
      }
    } catch (error) {
      healthChecks.analytics = { status: 'error', message: 'Analytics system unavailable' };
    }
    ...
    try {
      ...
      if (tablesWithQR > 0) {
        healthChecks.qrCodes = { status: 'healthy', message: `${tablesWithQR} QR codes active` };
      } else {
        healthChecks.qrCodes = { status: 'warning', message: 'No QR codes configured' };
      }
    } catch (error) {
      healthChecks.qrCodes = { status: 'error', message: 'QR code system unavailable' };
    }
    ...
    try {
      ...
      healthChecks.reservations = { status: 'error', message: 'Reservation system unavailable' };
    }
