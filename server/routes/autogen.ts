...
function pickColorsFromUrl(url: string | null) {
  ...
  if (u.includes("coffee") || u.includes("cafe") || u.includes("espresso"))
    return ["#D97706", "#FFFBEB"]; // amber
  if (u.includes("restaurant") || u.includes("food") || u.includes("menu"))
    return ["#DC2626", "#FFF1F2"]; // red
  if (u.includes("bar") || u.includes("cocktail"))
    return ["#7C3AED", "#EEF2FF"]; // purple
...
function detectPos(
  ...
  const text = [url, mapsLink, businessName]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
  ...
  for (const k of known) {
    if (text.includes(k))
      return {
        system: k.charAt(0).toUpperCase() + k.slice(1),
        status: "connected",
        capabilities: ["menu_sync", "inventory_sync"],
      };
  }
...
export const handleAutogen = async (req: Request, res: Response) => {
  try {
    ...
    if (heur.includes("cafe") || heur.includes("coffee"))
      business.category = "Café";
    else if (
      heur.includes("restaurant") ||
      heur.includes("resto") ||
      heur.includes("bistro")
    )
      business.category = "Restaurant";
    else if (heur.includes("bar") || heur.includes("pub"))
      business.category = "Bar";
    else if (heur.includes("gym") || heur.includes("fitness"))
    ...
    const seo = {
      ...
      keywords: [business.category, "local", "menu", "contact", business.name]
        .filter(Boolean)
        .slice(0, 10),
    };
