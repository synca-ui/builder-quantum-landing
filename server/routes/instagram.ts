...
export const fetchInstagramPhotos: RequestHandler = async (req, res) => {
  try {
    ...
    if (!profileUrl)
      return res.status(400).json({ error: "profileUrl query required" });
    ...
    if (!resp.ok)
      return res
        .status(502)
        .json({ error: "Failed to fetch Instagram profile" });
    ...
    if (images.length === 0) {
      const jsonMatch = html.match(
        /<script[^>]*>\s*window\.__additionalDataLoaded\('[^']+',\s*(\{[\s\S]*?\})\);\s*<\/script>/,
      );
